...
class Game extends CI_Controller {
    ...
    {
        $id = $this->Player_Model->user->login;
		$this->show('premiumPayment', $id);
    }
	...
	// ambasciata 
	function embassy($position = 0)
    {
	    $pos_type = 'pos'.$position.'_type';
        if ($position == 0 or $this->Player_Model->now_town->$pos_type != 11)
        {
        ...
        else
        {
	        ...
	        $query = $this->db->get($this->session->userdata('universe').'_alliance_users'); 
			...
			if ($query->num_rows() > 0) 
			{
			    ...
			    $ally_id = $row->ally_id;
			    $this->db->select('*');
	            $this->db->where('ally_id', $ally_id);
	            $query1 = $this->db->get($this->session->userdata('universe').'_alliance');
			    $row = $query1->row(); 
                $ally_int_page = $row->ally_int_page;
	            $this->show('embassy', $position, $ally_int_page, $ally_id);
            } else {
			    $this->show('embassy', $position);
			}   
        }	
	...
	function allyHighscore()
	{
        // All alliances
		$alliances = $this->db->get($this->session->userdata('universe').'_alliance');
	    
		// Select the user's alliance
		...
		if($query->num_rows() > 0)
	    {
	        $row = $query->row();
	        $ally_id = $row->ally_id;	 
	        $this->show('allyHighscore', $alliances, $ally_id);
	    } else {
	        $this->show('allyHighscore', $alliances);
	    }
	}
	
	// pag esterna
	function extpage($id_ally = 0)
	{
	if ($id_ally > 0)
	{
	$this->db->select('*');
	$this->db->where('ally_id', $id_ally);
	$query = $this->db->get($this->session->userdata('universe').'_alliance');
    $row = $query->row(); 
    $ally_tag = $row->ally_tag;
    $ally_name = $row->ally_name;
    $ally_ext_page = $row->ally_ext_page;
	$ally_description = $row->ally_description;
	
    $this->show('extpage', $ally_tag, $ally_name, $ally_ext_page, $ally_description, $id_ally);	
	}
	}
    
	//modifica pagina
	function editextpage($id_alleanza = 0)
	{
	if ($id_alleanza > 0)
	{
	$this->db->select('*');
	$this->db->where('ally_id', $id_alleanza);
	$query = $this->db->get($this->session->userdata('universe').'_alliance');
    $row = $query->row(); 
    $aly_ext_page = $row->ally_ext_page;    
	$this->show('editextpage', $aly_ext_page, $id_alleanza);
	}
	}
		
    // modifica pag interna
	function editintpage($id_alleanza = 0)
	{
	if ($id_alleanza > 0)
	{
	$this->db->select('*');
	...
	$query = $this->db->get($this->session->userdata('universe').'_alliance');
    $row = $query->row(); 
    $aly_int_page = $row->ally_int_page;    
	$this->show('editintpage', $aly_int_page, $id_alleanza);
	}
	}
	
	// modifica dati alleanza
	function editdati($id_alleanza)
	{
	if ($id_alleanza > 0)
	{
	$this->db->select('*');
	...
	$ally_tag = $row->ally_tag;
    $ally_description = $row->ally_description;
	$this->show('editdati', $ally_name, $ally_tag, $ally_description, $id_alleanza);	
	}
	}
	
	function sendAllyMex($id_alleanza)
	{
	if ($id_alleanza > 0)
	{
