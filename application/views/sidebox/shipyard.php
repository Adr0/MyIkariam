<div class="dynamic" id="reportInboxLeft">
    ...
    <div class="content">
        <div class="centerButton">
            <a href="<?=$this->config->item('base_url')?>game/fleetGarrisonEdit/" class="button"><?=$this->lang->line('fire_units')?></a>
        </div>
    </div>
...
<div id="unitConstructionList" class="dynamic"><h3 class="header"><?=$this->lang->line('buildings_construction_list')?></h3>
    <div class="content">
<?php if($this->Player_Model->armys[$this->Player_Model->town_id]->ships_line != ''){
    $ostalos_all = 0;
...
<?}?>
    ...
    <div style="text-align:center; padding: 10px 0 10px 0;">
        <a class="button" href="javascript:myConfirm('<?=$this->lang->line('build_cancel')?>','<?=$this->config->item('base_url')?>actions/abortShips/<?=$position?>/');"><?=$this->lang->line('cancel')?></a>
    </div>
<?}?>
